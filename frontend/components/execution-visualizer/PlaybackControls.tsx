    ...
    <div className="flex-shrink-0 border-t border-neu-border bg-neu-bg px-4 py-3 neu-raised-sm">
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          <label htmlFor="speed-select" className="text-sm text-neu-text-muted font-medium">
            Speed:
          </label>
          <select
            id="speed-select"
        ...
        <div className="flex items-center gap-2">
          {state.isPlaying && (
            <div className="flex items-center gap-1.5 text-sm text-neu-accent">
              <div className="w-2 h-2 bg-neu-accent rounded-full animate-pulse" />
              Playing
            </div>
          )}
          {!state.isPlaying && state.currentEventIndex >= totalEvents && (
            <div className="flex items-center gap-1.5 text-sm text-neu-green">
              <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
          ...
          {!state.isPlaying && state.currentEventIndex < totalEvents && state.currentEventIndex > 0 && (
            <div className="text-sm text-neu-text-muted">Paused</div>
          )}
        </div>
      </div>
    </div>
