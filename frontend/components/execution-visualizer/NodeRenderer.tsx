...
export function ExecutionNode({ data }: ExecutionNodeProps) {
  ...
  return (
    <div className={`px-4 py-2 rounded-lg border-2 min-w-[180px] transition-all ${getNodeStyle()}`}>
      ...
      {data.errorMessage && (
        <div className="text-xs text-red-600 mt-1 truncate" title={data.errorMessage}>
          {data.errorMessage}
        </div>
      )}
