    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6">
          <div className="text-3xl font-bold text-red-700">{summary.total_errors || 0}</div>
          <div className="text-sm text-red-600">Total Errors</div>
        </div>

        <div className="bg-orange-50 border border-orange-200 rounded-lg p-6">
          <div className="text-3xl font-bold text-orange-700">{summary.total_clusters || 0}</div>
          <div className="text-sm text-orange-600">Error Clusters</div>
        </div>

        <div className="bg-red-50 border border-red-200 rounded-lg p-6">
          <div className="text-3xl font-bold text-red-700">{summary.critical_count || 0}</div>
      ...
      {clusterList.length > 0 && (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {getPatternCounts(clusterList).map(({ pattern, count, color }) => (
              <div key={pattern || 'unknown'} className={`p-4 rounded-lg ${color}`}>
                <div className="text-2xl font-bold">{count}</div>
                <div className="text-sm capitalize">{formatPattern(pattern)}</div>
              </div>
            ))}
          </div>
        </div>
